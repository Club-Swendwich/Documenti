...
\begin{itemdescript}
    ...
    \item [To do] Lista delle card relative a issues$^{G}$ che devono ancora essere prese in carico. È il backlog della board.
    ...
    \item [Done] Lista degli issues$^{G}$ approvati e risolti.
\end{itemdescript}
...
Tale issue$^{G}$ dovrà essere aperta su GitHub$^{G}$ nell'apposita sezione "Issues"$^{G}$, della repository$^{G}$ Documenti e dovrà essere compilata nel seguente modo:
\begin{itemize}
...
\noindent Le issues create in questo modo saranno automaticamente trasformate in \textit{Trello$^{G}$ cards} grazie all'integrazione con Zapier$^{G}$. \\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Legenda: posizionata a destra; colore del testo nero; dimensione$^{G}$ 16 pt.
...
\mbox{}\\
Tutta la documentazione prodotta deve essere mantenuta all'interno della repo git$^{G}$ hostata su GitHub$^{G}$ \href{https://github.com/Club-Swendwich/Documenti}{(https://github.com/Club-Swendwich/Documenti)}.
...
\begin{itemize}
    ...
    \item \texttt{template}: Contiene tutti i template$^{G}$ da utilizzare quando si va a
